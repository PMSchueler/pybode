...
class PYDG1000(FunctionGenerator):
    def __init__(
        self,
    ...
    def _connect(self, address = None, port = None):
        if self._scpi.connect(address, port):
            ...
            idnString = self._idn()
            if not idnString.startswith("Rigol Technologies,DG1022Z"):
                self._disconnect()
    ...
    def _set_channel_enabled(self, channel, enabled):
        if (channel < 0) or (channel > 1):
            raise ValueError("Invalid channel number for DG1000")
        if enabled:
            self._scpi.scpiCommand(f":OUTP{channel+1}:LOAD 50")
            self._scpi.scpiCommand(f":OUTP{channel+1}:STAT ON")
        else:
            self._scpi.scpiCommand(f":CHAN{channel+1}:DISP INF")
            self._scpi.scpiCommand(f":OUTP{channel+1}:STAT OFF")
    ...
    def _set_channel_waveform(self, channel, waveform, arbitrary = None):
        ...
        self._scpi.scpiCommand(f":SOUR{channel+1}:FUNC:SHAP: {self._channel_waveform[waveform]}")
    ...
    def set_model(self):
        ...
        print(f":PROJ:STAT MODEL,DG1062Z")
        readData = ""        
        while True:
